...
pub fn get_area_by_journey_bitmap_interation_bit(journey_bitmap: &JourneyBitmap) -> f64 {
...
pub fn get_area_by_journey_bitmap_interation_bit_width_only(journey_bitmap: &JourneyBitmap) -> f64 {
    let total_area: f64 = journey_bitmap
        .tiles
        .iter()
        .flat_map(|(tile_pos, tile)| {
...
pub fn get_area_by_journey_bitmap_interation_block(journey_bitmap: &JourneyBitmap) -> f64 {
    let total_area: f64 = journey_bitmap
        .tiles
        .iter()
        .flat_map(|(tile_pos, tile)| {
            tile.blocks.iter().filter_map(move |(block_pos, block)| {
                ...
                if bit_count > 0 {
                    ...
                    /* height=R⋅Δφ, where Δφ = φ2-φ1 is the difference in latitudes in radians. */
                    let height = EARTH_RADIUS * (lat2 - lat1).abs().to_radians();
                    let block_area = avg_width * height * bit_count as f64 / (BITMAP_WIDTH * BITMAP_WIDTH) as f64;
                    Some(block_area)
                }
                else {
                    None
